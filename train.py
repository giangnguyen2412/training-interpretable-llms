...
def get_batch(split):
    ...
    # Get original text for attribution
    texts = []
    for i in ix:
        text_data = data[i:i + block_size]
        # text = "".join([chr(int(c)) for c in text_data])  # Adjust based on your tokenizer
        text = text_data
        texts.append(text)
...
# training loop
X, Y, texts = get_batch('train') # fetch the very first batch
t0 = time.time()
